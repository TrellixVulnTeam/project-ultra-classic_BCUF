...
Function un.UninstallServiceIfNotUsed
  ...
  StrCpy $0 0
  ${Do}
    EnumRegKey $1 HKLM "Software\GamesandOSStudio\MaintenanceService" $0
    ${If} "$1" == ""
      ${ExitDo}
    ${EndIf}
    IntOp $0 $0 + 1
...
Section "Uninstall"
  ...
  ${un.InitHashAppModelId} "$INSTDIR" "Software\GamesandOSStudio\${AppName}\TaskBarIDs"

  SetShellVarContext current  ; Set SHCTX to HKCU
  ${un.RegCleanMain} "Software\GamesandOSStudio"
  ...
  ${un.CleanUpdateDirectories} "GamesandOSStudio\ProjectUltraClassic" "GamesandOSStudio\updates"

  ; Remove any app model id's stored in the registry for this install path
  DeleteRegValue HKCU "Software\GamesandOSStudio\${AppName}\TaskBarIDs" "$INSTDIR"
  DeleteRegValue HKLM "Software\GamesandOSStudio\${AppName}\TaskBarIDs" "$INSTDIR"
  ...
  ${un.RegCleanAppHandler} "PJUCURL"
  ${un.RegCleanAppHandler} "PJUCHTML"
  ${un.RegCleanProtocolHandler} "ftp"
  ${un.RegCleanProtocolHandler} "http"
  ${un.RegCleanProtocolHandler} "https"

  ClearErrors
  ReadRegStr $R9 HKCR "PJUCHTML" ""
  ; Don't clean up the file handlers if the PJUCHTML key still exists since
  ; there should be a second installation that may be the default file handler
  ${If} ${Errors}
    ${un.RegCleanFileHandler}  ".htm"   "PJUCHTML"
    ${un.RegCleanFileHandler}  ".html"  "PJUCHTML"
    ${un.RegCleanFileHandler}  ".shtml" "PJUCHTML"
    ${un.RegCleanFileHandler}  ".xht"   "PJUCHTML"
    ${un.RegCleanFileHandler}  ".xhtml" "PJUCHTML"
    ${un.RegCleanFileHandler}  ".oga"  "PJUCHTML"
    ${un.RegCleanFileHandler}  ".ogg"  "PJUCHTML"
    ${un.RegCleanFileHandler}  ".ogv"  "PJUCHTML"
    ${un.RegCleanFileHandler}  ".pdf"  "PJUCHTML"
    ${un.RegCleanFileHandler}  ".webm"  "PJUCHTML"
  ${EndIf}

  SetShellVarContext all  ; Set SHCTX to HKLM
  ${un.GetSecondInstallPath} "Software\GamesandOSStudio" $R9
  ${If} $R9 == "false"
    SetShellVarContext current  ; Set SHCTX to HKCU
    ${un.GetSecondInstallPath} "Software\GamesandOSStudio" $R9
  ${EndIf}
  ...
  WriteRegStr HKCU "Software\GamesandOSStudio\ProjectUltraClassic" "Uninstalled-${UpdateChannel}" "True"
