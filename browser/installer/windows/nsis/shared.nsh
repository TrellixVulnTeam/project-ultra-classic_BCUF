...
!macro PostUpdate
  ...
  ${AndIf} $9 != 0 ; We're not running in session 0
    ReadRegStr $0 HKCU "Software\Classes\PJUCURL\shell\open\command" ""
  ...
  ; install location in the Software\GamesandOSStudio key and uninstall registry entries
  ; that point to our install location for both HKCU and HKLM.
  SetShellVarContext current  ; Set SHCTX to the current user (e.g. HKCU)
  ${RegCleanMain} "Software\GamesandOSStudio"
  ${RegCleanUninstall}
  ${UpdateProtocolHandlers}

  ; setup the application model id registration value
  ${InitHashAppModelId} "$INSTDIR" "Software\GamesandOSStudio\${AppName}\TaskBarIDs"

  ; Win7 taskbar and start menu link maintenance
  Call FixShortcutAppModelIDs

  ClearErrors
  WriteRegStr HKLM "Software\GamesandOSStudio" "${BrandShortName}InstallerTest" "Write Test"
  ${If} ${Errors}
    StrCpy $TmpVal "HKCU" ; used primarily for logging
  ${Else}
    SetShellVarContext all    ; Set SHCTX to all users (e.g. HKLM)
    DeleteRegValue HKLM "Software\GamesandOSStudio" "${BrandShortName}InstallerTest"
    ...
    ReadRegStr $0 HKLM "Software\gamesandosstudio.com\GamesandOSStudio" "CurrentVersion"
    ${If} "$0" != "${GREVersion}"
      WriteRegStr HKLM "Software\gamesandosstudio.com\GamesandOSStudio" "CurrentVersion" "${GREVersion}"
    ${EndIf}
  ${EndIf}
...
!ifdef MOZ_MAINTENANCE_SERVICE
  ...
  ${AndIf} ${AtLeastWinXP}
    ...
    ${If} ${RunningX64}
      SetRegView 64
    ${EndIf}
    ReadRegDWORD $5 HKLM "Software\GamesandOSStudio\MaintenanceService" "Attempted"
    ClearErrors
    ${If} ${RunningX64}
      SetRegView lastused
    ${EndIf}
...
!macro AddAssociationIfNoneExist FILE_TYPE
  ...
  ${If} ${Errors}
    WriteRegStr SHCTX "SOFTWARE\Classes\${FILE_TYPE}"  "" "PJUCHTML"
  ${EndIf}
  WriteRegStr SHCTX "SOFTWARE\Classes\${FILE_TYPE}\OpenWithProgids" "PJUCHTML" ""
!macroend
...
!macro SetHandlers
  ...
  ; Associate the file handlers with PJUCHTML
  ReadRegStr $6 SHCTX "$0\.htm" ""
  ${If} "$6" != "PJUCHTML"
    WriteRegStr SHCTX "$0\.htm"   "" "PJUCHTML"
  ${EndIf}

  ReadRegStr $6 SHCTX "$0\.html" ""
  ${If} "$6" != "PJUCHTML"
    WriteRegStr SHCTX "$0\.html"  "" "PJUCHTML"
  ${EndIf}

  ReadRegStr $6 SHCTX "$0\.shtml" ""
  ${If} "$6" != "PJUCHTML"
    WriteRegStr SHCTX "$0\.shtml" "" "PJUCHTML"
  ${EndIf}

  ReadRegStr $6 SHCTX "$0\.xht" ""
  ${If} "$6" != "PJUCHTML"
    WriteRegStr SHCTX "$0\.xht"   "" "PJUCHTML"
  ${EndIf}

  ReadRegStr $6 SHCTX "$0\.xhtml" ""
  ${If} "$6" != "PJUCHTML"
    WriteRegStr SHCTX "$0\.xhtml" "" "PJUCHTML"
  ${EndIf}
  ...
  ; An empty string is used for the 5th param because PJUCHTML is not a
  ; protocol handler
  ${AddDisabledDDEHandlerValues} "PJUCHTML" "$2" "$8,1" \
                                 "${AppRegName} HTML Document" ""

  ${AddDisabledDDEHandlerValues} "PJUCURL" "$2" "$8,1" "${AppRegName} URL" \
                                 "true"
...
!macro SetStartMenuInternet RegKey
  ...
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".htm"   "PJUCHTML"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".html"  "PJUCHTML"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".shtml" "PJUCHTML"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".xht"   "PJUCHTML"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".xhtml" "PJUCHTML"

  WriteRegStr ${RegKey} "$0\Capabilities\StartMenu" "StartMenuInternet" "$R9"

  WriteRegStr ${RegKey} "$0\Capabilities\URLAssociations" "ftp"    "PJUCURL"
  WriteRegStr ${RegKey} "$0\Capabilities\URLAssociations" "http"   "PJUCURL"
  WriteRegStr ${RegKey} "$0\Capabilities\URLAssociations" "https"  "PJUCURL"
...
; The IconHandler reference for PJUCHTML can end up in an inconsistent state
; due to changes not being detected by the IconHandler for side by side
; installs (see bug 268512). The symptoms can be either an incorrect icon or no
; icon being displayed for files associated with Firefox (does not use SHCTX).
!macro FixShellIconHandler RegKey
  ClearErrors
  ReadRegStr $1 ${RegKey} "Software\Classes\PJUCHTML\ShellEx\IconHandler" ""
  ${Unless} ${Errors}
    ReadRegStr $1 ${RegKey} "Software\Classes\PJUCHTML\DefaultIcon" ""
    ${GetLongPath} "$INSTDIR\${FileMainEXE}" $2
    ${If} "$1" != "$2,1"
      WriteRegStr ${RegKey} "Software\Classes\PJUCHTML\DefaultIcon" "" "$2,1"
    ${EndIf}
  ${EndUnless}
!macroend
!define FixShellIconHandler "!insertmacro FixShellIconHandler"

; Add Software\GamesandOSStudio\ registry entries (uses SHCTX).
!macro SetAppKeys
  ...
  StrCpy $0 "Software\GamesandOSStudio\${BrandFullNameInternal}\${AppVersion}$3 (${ARCH} ${AB_CD})\Main"
  ${WriteRegStr2} $TmpVal "$0" "Install Directory" "$8" 0
  ${WriteRegStr2} $TmpVal "$0" "PathToExe" "$8\${FileMainEXE}" 0

  StrCpy $0 "Software\GamesandOSStudio\${BrandFullNameInternal}\${AppVersion}$3 (${ARCH} ${AB_CD})\Uninstall"
  ${WriteRegStr2} $TmpVal "$0" "Description" "${BrandFullNameInternal} ${AppVersion}$3 (${ARCH} ${AB_CD})" 0

  StrCpy $0 "Software\GamesandOSStudio\${BrandFullNameInternal}\${AppVersion}$3 (${ARCH} ${AB_CD})"
  ...
  StrCpy $0 "Software\GamesandOSStudio\${BrandFullNameInternal} ${AppVersion}$3\bin"
  ${WriteRegStr2} $TmpVal "$0" "PathToExe" "$8\${FileMainEXE}" 0

  StrCpy $0 "Software\GamesandOSStudio\${BrandFullNameInternal} ${AppVersion}$3\extensions"
  ${WriteRegStr2} $TmpVal "$0" "Components" "$8\components" 0
  ${WriteRegStr2} $TmpVal "$0" "Plugins" "$8\plugins" 0

  StrCpy $0 "Software\GamesandOSStudio\${BrandFullNameInternal} ${AppVersion}$3"
  ...
  StrCpy $0 "Software\GamesandOSStudio\${BrandFullNameInternal}$3"
...
; to PJUCHTML when there is a value for PersistentHandler or by removing the
...
!macro FixBadFileAssociation FILE_TYPE
  ...
  ${If} "$2" != ""
    ; Since there is a persistent handler remove PJUCHTML as the default
    ; value from both HKCU and HKLM if it set to PJUCHTML.
    ${If} "$0" == "PJUCHTML"
    ...
    ${EndIf}
  ${ElseIf} "$0" == "PJUCHTML"
    ; Since KHCU is set to PJUCHTML remove PJUCHTML as the default value
...
!macro UpdateProtocolHandlers
  ...
  ${IsHandlerForInstallDir} "PJUCHTML" $R9
  ${If} "$R9" == "true"
    ; An empty string is used for the 5th param because PJUCHTML is not a
    ; protocol handler.
    ${AddDisabledDDEHandlerValues} "PJUCHTML" "$2" "$8,1" \
  ...
  ${IsHandlerForInstallDir} "PJUCURL" $R9
  ${If} "$R9" == "true"
    ${AddDisabledDDEHandlerValues} "PJUCURL" "$2" "$8,1" \
                                   "${AppRegName} URL" "true"
  ${EndIf}
...
!macro AddMaintCertKeys
  ...
  ${If} $R0 != ""
    ...
    WriteRegDWORD HKLM "Software\GamesandOSStudio\MaintenanceService" "Attempted" 1
...
!macro RemoveDeprecatedKeys
  ...
  ; Delete gopher from the user's UrlAssociations if it points to PJUCURL.
