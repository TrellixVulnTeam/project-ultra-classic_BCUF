...
Section "-InstallStartCleanup"
  ...
  ${CleanUpdateDirectories} "GamesandOSStudio\ProjectUltraClassic" "GamesandOSStudio\updates"
...
Section "-Application" APP_IDX
  ...
  ; On install always add the PJUCHTML and PJUCURL keys.
  ; An empty string is used for the 5th param because PJUCHTML is not a
  ; protocol handler.
  ${GetLongPath} "$INSTDIR\${FileMainEXE}" $8
  StrCpy $2 "$\"$8$\" -osint -url $\"%1$\""

  ; In Win8, the delegate execute handler picks up the value in PJUCURL and
  ; PJUCHTML to launch the desktop browser when it needs to.
...
Section "-InstallEndCleanup"
  ...
  ${Unless} ${Silent}
    ...
    ${If} "$0" == "1"
      ...
      ${If} "$0" != ""
        ClearErrors
        WriteRegStr HKCU "Software\GamesandOSStudio\ProjectUltraClassic" "OldDefaultBrowserCommand" "$0"
      ${EndIf}
    ...
    ${ElseIfNot} ${Errors}
      ...
      ClearErrors
      WriteRegStr HKCU "Software\GamesandOSStudio\ProjectUltraClassic" "DefaultBrowserOptOut" "True"
      ${If} ${Errors}
...
Function preComponents
  ...
  ClearErrors
  WriteRegStr HKLM "Software\GamesandOSStudio" \
...
Function preSummary
  ...
  ${Unless} ${Errors}
    DeleteRegValue HKLM "Software\GamesandOSStudio" "${BrandShortName}InstallerTest"
    ; Check if Project Ultra Classic is the http handler for this user.
    SetShellVarContext current ; Set SHCTX to the current user
    ${IsHandlerForInstallDir} "http" $R9
    ${If} $TmpVal == "HKLM"
      SetShellVarContext all ; Set SHCTX to all users
    ${EndIf}
    ; If Project Ultra Classic isn't the http handler for this user show the option to set
    ; Project Ultra Classic as the default browser.
    ${If} "$R9" != "true"
